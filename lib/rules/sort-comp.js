...
function getRangeToReplace(tokenBeforePrevProp, currProp) {
  return [tokenBeforePrevProp.end, currProp.end];
}
        ...
        if (!isValidOrder(prevName, currName)) {
          context.report({
            ...
            fix(fixer) {
              ...
              const textBetweenProps = sourceCode.getText(tokenBeforeCurrProp);

              const textToReplace = currPropText + textBetweenProps + prevPropText;
